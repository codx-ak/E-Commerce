  ...
  useEffect(()=>{
    function FilteredProducts(){
      ...
      if(buttonSelect==='Mobiles'){
        const mobileFilter=Product.filter(value=>value.category==='smartphones')
        setProducts(mobileFilter)
      }
      if(buttonSelect==='Laptops'){
        const laptopFilter=Product.filter(value=>value.category==='laptops')
        setProducts(laptopFilter)
      }
    }
    FilteredProducts()
  },[buttonSelect])
    ...
    <Container>
      ...
      <Box>
        ...
        >
          ...
          <MenuItem value={40}>Highest</MenuItem>
        </Select>
      </FormControl>
          </Box>
        </Box>
        <Box sx={{display:'flex',flexWrap:'wrap',gap:'5px',marginTop:4}}>
        {Product.length ? filteredProduct.map((product,index)=>{
